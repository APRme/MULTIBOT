...
升级后启动首行应为 `MULTIBOT V26.3.16-WareHouse`；如果仍显示旧版本号，应先检查当前 Git 提交和实际启动目录。
...
### 4.13 `warehouse`(仓库分拣假人)

仓库功能让 bot 承担"按规则把暂存箱/背包里的物品自动分拣到指定箱子"的自动化，并维护一份"容器坐标 ↔ 物品/数量"的持久化索引。代码位于 `src/warehouse/`(独立于 `src/features/`)。

#### 配置段

| 字段 | 类型 | 代码默认 | 说明 |
| --- | --- | --- | --- |
| `warehouse.enabled` | boolean | `false` | 是否启用仓库功能；启用后还需要 `capabilities.terrainHandling = true`(寻路/开箱) |
| `warehouse.rulesFile` | string | `rules.json` | 相对于 `WareHouse/<serverDir>/` 的规则路径；统一使用 `/` 分隔 |
...
仓库移动和自动化参数属于服务器共享规则，统一配置在 `rules.json`，不再从 Bot 的 `config.json` 读取。

#### 数据与规则文件位置

- 规则文件：`WareHouse/<serverDir>/<warehouseId>/rules.json`——**手动编辑**，命令只负责加载/查看/重载
- 数据库：与规则文件位于同一仓库目录，例如 `WareHouse/EDEN/wh1/warehouse.db`
- 同一服务器内，配置相同 `rulesFile` 的 Bot 共享规则、数据库与任务队列；配置 `wh1/rules.json` 和 `wh2/rules.json` 的 Bot 使用相互隔离的仓库
- `rulesFile` 必须是相对于 `WareHouse/<serverDir>/` 的路径，只允许 `/` 分隔；绝对路径、反斜杠和包含 `..` 的路径会被拒绝
- 注意：`WareHouse/` 整目录被 `.gitignore` 排除，真实规则与数据库属于本地数据，不会进入 git

#### `rules.json` 格式

| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `movement.lockY.enabled` | boolean | 否 | 默认 `false`；是否让仓库任务整条路径固定在指定 Y 层，只影响仓库移动；楼梯、半砖等碰撞面产生的合法半格站位会按其所在的锁定层处理 |
| `movement.lockY.value` | integer | 否 | 默认 `74`；启用后仓库移动锁定在该逻辑高度，意外脱离时会自动执行一次内置恢复，无需额外配置 |
| `idle.enabled` | boolean | 否 | 默认 `false`；队列空闲时是否前往挂机位置 |
| `idle.position` | `{x,y,z}` | `idle.enabled=true` 时是 | 挂机位置；启用锁高时其 Y 必须等于 `movement.lockY.value` |
| `automation.pickupSort.enabled` | boolean | 否 | 默认 `false`；Bot 自然拾取物品后是否自动延迟入队一次 `sort` |
| `automation.pickupSort.delaySeconds` | number | 否 | 默认 `10`；最后一次拾取后等待秒数，连续拾取会重置倒计时 |
| `automation.scheduled.enabled` | boolean | 否 | 默认 `false`；是否启用固定间隔维护任务 |
| `automation.scheduled.intervalSeconds` | number | 否 | 默认 `1800`；维护间隔秒数，最小按 1 秒处理 |
| `automation.scheduled.action` | string | 否 | 默认 `sortThenAudit`；可选 `sort`、`audit`、`sortThenAudit` |
| `inbox` | `[{x,y,z}]` | 否 | 暂存箱（玩家放待分拣物品），可多个 |
| `dropZone` | `{min:{x,y,z}, max:{x,y,z}}` | 否 | 掉落物登记区（当前仅登记，不主动拾取） |
| `containers` | `[...]` | 是 | 目标箱清单，至少 1 个 |
| `containers[].name` | string | 是 | 箱子名称（唯一标识，audit 单箱参数用） |
| `containers[].x / y / z` | integer | 是 | 坐标；`containers` 之间、以及与 `inbox` / `pickup` 不能重复 |
| `containers[].allow` | string[] | 否 | 允许放入的物品名（`minecraft:` 前缀可省，大小写不敏感） |
| `containers[].default` | boolean | 否 | 杂项兜底箱，全规则最多 1 个 |
| `pickup` | `{x,y,z}` | 否 | 取货箱：`withdraw` 取出后自动投递到此箱 |

所有 `x/y/z` 都是容器方块的真实坐标，不是 Bot 站位。启用 `movement.lockY` 后，程序会在锁定高度上搜索容器周围四格内的站位，以 Bot 眼睛到容器方块最近表面的距离校验 4 格交互范围，并先排除脚部或头部被方块占据的位置；地毯会作为可行走薄层处理，楼梯、半砖等非整格碰撞面只要实际站立面仍属于锁定层也会被接受。某个站位无路可达时会尝试下一个。如果容器在该高度上超出交互范围，任务会明确失败，且不会为接近容器而离开锁定高度。Bot 意外离开锁定层或寻路器报告卡住时，会优先在 6 格内寻找最近的完整锁高站位，找不到再尝试 12 格内最近一次成功站位；恢复路径只能经过锁定层及其下一格，8 秒内仍失败就停止本次任务，不会任意跨层。

修改这些运行参数后执行 `warehouse reload` 即可与容器规则一起重新加载；非法类型、无效挂机坐标或挂机高度与锁定高度不一致时，整份规则会被拒绝。

匹配顺序：**精确命中 `allow` → `default` 兜底箱 → 无匹配则滞留**（物品留在暂存箱/背包并在任务结果中报告）。

完整示范见 `MULTIBOT/examples/full-config-example/warehouse/EDEN/wh1/rules.json`，对应运行时的 `MULTIBOT/WareHouse/EDEN/wh1/rules.json`。

#### 命令（whisper / HTTP / console 统一入口）

| 命令 | 类型 | 说明 |
| --- | --- | --- |
| `warehouse help / rules / reload` | 即时 | 帮助、规则摘要、重载规则（改完 rules.json 后执行） |
| `warehouse inspect <x> <y> <z>` | 即时 | 开箱读取并写入索引 |
| `warehouse query [物品名]` | 即时 | 无参=跨箱汇总；带参=该物品在哪些容器、各多少 |
| `warehouse sort` | 入队 | 分拣：先清背包（自然拾取/遗留物按规则入库），再逐暂存箱分拣 |
| `warehouse audit [箱名]` | 入队 | 盘点：对照游戏实际修正索引；带箱名只盘该箱 |
| `warehouse withdraw <物品> [数量]` | 入队 | 取出：优先投递 `pickup` 取货箱，否则走到请求者附近丢出，兜底留在背包 |
| `warehouse task list` / `warehouse task cancel <id>` | 即时 | 查看队列 / 取消任务 |

#### 行为说明

- **分拣**：跨容器搬运经 **bot 背包中转**（暂存箱 → 背包 → 目标箱）；整组物品使用服务端 `QUICK_MOVE`（等同 Shift+左键），由原版容器规则决定合并或空槽，部分数量和失败后放回原槽才使用定点点击；目标箱满则物品放回原处并报告滞留。
- **搬运诊断**：开发阶段每次搬运会输出 `[WAREHOUSE][MOVE]`，包括方向、容器坐标、源槽、实际目标槽、数量、`stackSize`、截短身份哈希和恢复结果；任务结束会输出背包残留摘要。日志不会输出完整 NBT、components 或账号凭据。
- **盘点**：与索引比对，差异以游戏实际为准修正并更新 `last_audited_at`；物品身份包含完整 `components`、`removedComponents` 和旧版 NBT，同 ID、同数量但组件不同也会报告 `components_changed`；旧数据库首次盘点会静默回填身份；差异经 SSE `warehouse audit_delta` 事件发布；**断点续盘**：中断后重启自动从上次进度继续。
- **任务队列**：`sort` / `audit` / `withdraw` 统一入队，**单 bot 串行执行**；断线/重启时 `running → interrupted`，重连后自动转 `queued` 续跑；可取消。
- **容器站位**：未锁定高度时使用 `GoalLookAtBlock` 寻找距离不超过 4 格且能看见箱子的位置；锁定高度时搜索容器周围四格内的同层空位，并按眼睛到方块最近表面的距离判断是否可交互；到达后通过世界方块射线确认视线直接命中容器，被树叶、玻璃或其他方块遮挡时继续尝试其他候选，真正交互前仍会再次校验，绝不隔墙发送交互包。
- **空闲自动化**：启用 `idle` 后，任务队列清空时 Bot 返回指定位置；启用 `pickupSort` 后，Bot 拾取物品会在最后一次拾取后的延迟结束时自动执行一次 `sort`；定时维护可按 `scheduled.action` 选择分拣、盘点或先分拣再全库盘点。
- **SSE 事件**：`event: warehouse`，payload `{ botId, serverDir, timestamp, type, data }`；`type` ∈ `task_status` / `audit_delta`（经全局 `GET /api/events` 推送）。

#### 注意事项

- 依赖 `capabilities.terrainHandling = true`；`entityHandling` 无要求。
- legacy ASSN 模式（无 `serverDir`）不支持，启用无效。
- `node:sqlite` 是 Node 22 的实验性 API，启动会有 `ExperimentalWarning`，不影响功能。
- 规则校验失败（缺坐标、坐标重复、多个 default 等）会**拒绝加载**并报具体错误，`warehouse rules` 可查看错误。
- 掉落物**不主动追踪拾取**：玩家丢出的物品只有被 Bot 自然捡到后才进入背包；启用 `automation.pickupSort` 时会在设定延迟后自动入队一次 `sort`，否则等待手动或定时分拣。
...
- `warehouse/EDEN/wh1/rules.json`：仓库分拣规则完整示范，对应运行时的 `WareHouse/EDEN/wh1/rules.json`
...
- `recording.enabled`
- `ScriptScheduler`
- `warehouse.enabled`（规则文件见 `warehouse/EDEN/wh1/rules.json`，详细说明见 4.13）
