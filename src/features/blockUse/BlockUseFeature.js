...
class BlockUseFeature {
  ...
  isBlockVisible(block) {
    if (!block || !this.bot || typeof this.bot.canSeeBlock !== 'function') {
      return false;
    }
    return this.bot.canSeeBlock(block) === true;
  }

  isBlockVisibleAt(x, y, z) {
    if (!this.bot || typeof this.bot.blockAt !== 'function') {
      return false;
    }
    return this.isBlockVisible(this.bot.blockAt(new Vec3(x, y, z)));
  }

  ensureBlockVisible(block) {
    if (!this.isBlockVisible(block)) {
      const { x, y, z } = block.position;
      throw new Error(`目标方块被遮挡，无法交互 (${x}, ${y}, ${z})`);
    }
  }

  // 面向并激活方块(开箱/开门)。失败抛错,由调用方处理。
  async openBlock(x, y, z) {
    ...
    this.ensureBlockVisible(targetBlock);
