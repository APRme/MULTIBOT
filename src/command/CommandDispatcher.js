...
class CommandDispatcher {
  constructor(options = {}) {
    ...
    this.warehouseFeature = options.warehouseFeature;
  }
  ...
  getHelpLines() {
    ...
    return [
      ...
      withCapability('warehouse <help|rules|reload|inspect|query|sort|audit|withdraw|task>', { terrainHandling: true }),
      '  changeslot <1-9|info>',
      '  look <yaw> <pitch>',
  ...
  async dispatch(content, context) {
    ...
    if (command === 'warehouse') {
      if (!this.ensureTerrainHandling(context, 'warehouse')) {
        return true;
      }

      if (!this.warehouseFeature || typeof this.warehouseFeature.handleWarehouseCommand !== 'function') {
        context.replyError('仓库模块未初始化');
        return true;
      }

      await this.warehouseFeature.handleWarehouseCommand(context, trimmed);
      return true;
    }
