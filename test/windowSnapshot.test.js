...
test('serializeItem extracts base fields only', () => {
  ...
  assert.deepEqual(serializeItem(item), {
    ...
    durabilityUsed: null,
    maxDurability: null,
    stackIdentity: getItemIdentityHash(item)
  });
});
    ...
    slots: [
      {
        ...
        durabilityUsed: null,
        maxDurability: null,
        stackIdentity: getItemIdentityHash(window.slots[1])
      }
    ]
  });
});

test('serializeItem preserves component-aware stack identity', () => {
  const alpha = {
    slot: 0,
    type: 1029,
    name: 'book',
  ...
  assert.notEqual(serializeItem(alpha).stackIdentity, serializeItem(beta).stackIdentity);
});
