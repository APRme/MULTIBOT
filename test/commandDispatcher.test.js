...
function createDispatcher(calls, overrides = {}) {
  ...
  return new CommandDispatcher({
    ...
    ...(overrides.omitWarehouse === true ? {} : {
      warehouseFeature: {
        async handleWarehouseCommand(context, trimmed) {
          calls.push(['warehouse', trimmed]);
        },
        ...(overrides.warehouseFeature || {})
      }
    })
  });
}
...
test('CommandDispatcher routes warehouse commands to warehouseFeature', async () => {
  const calls = [];
  const dispatcher = createDispatcher(calls);
  const context = createContext();

  const handled = await dispatcher.dispatch('warehouse help', context);

  assert.equal(handled, true);
  assert.ok(calls.some((entry) => entry[0] === 'warehouse' && entry[1] === 'warehouse help'));
});

test('CommandDispatcher replies 仓库模块未初始化 when warehouseFeature missing', async () => {
  const calls = [];
  const dispatcher = createDispatcher(calls, { omitWarehouse: true });
  const context = createContext();

  const handled = await dispatcher.dispatch('warehouse help', context);

  assert.equal(handled, true);
  assert.ok(context.messages.some((entry) => entry.type === 'error' && entry.message.includes('仓库模块未初始化')));
});
