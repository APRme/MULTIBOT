...
test('chest move treats different item components as incompatible', async () => {
  const { feature, bot } = createFeature();
  const chest = createMockWindow({ id: 3 });
  chest.slots = [
    createItem(0, 'book', 4, { components: [{ type: 'minecraft:custom_name', data: 'alpha' }] }),
    createItem(1, 'book', 1, { components: [{ type: 'minecraft:custom_name', data: 'beta' }] })
  ];
  bot.currentWindow = chest;
  feature.attach(bot);

  await assert.rejects(
    feature.moveSlotItem(0, 1, 1),
    /目标槽位 1 有不同类型的物品/
  );
});

test('chest move reads prismarine item stackSize for non-stackable items', async () => {
  const { feature, bot } = createFeature();
  const chest = createMockWindow({ id: 3 });
  chest.slots = [
    createItem(0, 'flint_and_steel', 1, { stackSize: 1, maxStackSize: undefined }),
    createItem(1, 'flint_and_steel', 1, { stackSize: 1, maxStackSize: undefined })
  ];
  bot.currentWindow = chest;
  installClickSimulator(bot);
  feature.attach(bot);
  ...
  await assert.rejects(
    feature.moveSlotItem(0, 1, 1, { preventSwap: true }),
    /目标槽位 1 无法容纳 1 个/
  );
  assert.equal(chest.slots[0].count, 1);
  assert.equal(chest.slots[1].count, 1);
});

test('chest move quickMove delegates placement to the server target range', async () => {
  const { feature, bot } = createFeature();
  const chest = createMockWindow({ id: 3 });
  chest.slots = new Array(46).fill(null);
  chest.slots[0] = createItem(0, 'flint_and_steel', 1, { stackSize: 1, maxStackSize: undefined });
  bot.currentWindow = chest;
  bot.clickWindow = async (slot, button, mode) => {
    assert.equal(button, 0);
    assert.equal(mode, 1);
    const source = chest.slots[slot];
    const start = slot < chest.inventoryStart ? chest.inventoryStart : 0;
    const end = slot < chest.inventoryStart ? chest.inventoryEnd : chest.inventoryStart;
    for (let target = start; target < end; target += 1) {
      if (!chest.slots[target]) {
        chest.slots[target] = { ...source, slot: target };
        chest.slots[slot] = null;
        return;
      }
    }
    throw new Error('目标区已满');
  };
  feature.attach(bot);

  const result = await feature.moveSlotItem(0, 27, 1, { quickMove: true });
  assert.equal(result.ok, true);
  assert.equal(result.destinationSlot, 27);
  assert.equal(chest.slots[0], null);
  assert.equal(chest.slots[27].name, 'flint_and_steel');
});

test('chest move quickMove detects in-place count increases', async () => {
  const { feature, bot } = createFeature();
  const chest = createMockWindow({ id: 3 });
  chest.slots = new Array(46).fill(null);
  chest.slots[0] = createItem(0, 'iron_ingot', 10, { stackSize: 64 });
  chest.slots[27] = createItem(27, 'iron_ingot', 30, { stackSize: 64 });
  bot.currentWindow = chest;
  bot.clickWindow = async (slot, button, mode) => {
    assert.equal(button, 0);
    assert.equal(mode, 1);
    chest.slots[27].count += chest.slots[slot].count;
    chest.slots[slot] = null;
  };
  feature.attach(bot);

  const result = await feature.moveSlotItem(0, 27, 10, { quickMove: true });
  assert.equal(result.ok, true);
  assert.equal(result.destinationSlot, 27);
  assert.equal(chest.slots[27].count, 40);
});

test('chest move quickMove skips same ID stacks with different components', async () => {
  const { feature, bot } = createFeature();
  const chest = createMockWindow({ id: 3 });
  chest.slots = new Array(46).fill(null);
  const alpha = createItem(0, 'book', 1, {
    stackSize: 64,
    components: [{ type: 'minecraft:custom_name', data: 'alpha' }]
  });
  const beta = createItem(27, 'book', 1, {
    stackSize: 64,
    components: [{ type: 'minecraft:custom_name', data: 'beta' }]
  });
  chest.slots[0] = alpha;
  chest.slots[27] = beta;
  bot.currentWindow = chest;
  bot.clickWindow = async (slot, button, mode) => {
    ...
    assert.equal(mode, 1);
    const source = chest.slots[slot];
    for (let target = chest.inventoryStart; target < chest.inventoryEnd; target += 1) {
      const existing = chest.slots[target];
      if (existing && existing.type === source.type && existing.count < existing.stackSize) {
  ...
  const result = await feature.moveSlotItem(0, 27, 1, { quickMove: true });
  assert.equal(result.ok, true);
  assert.equal(result.destinationSlot, 28);
  assert.equal(chest.slots[27].count, 1);
  assert.equal(chest.slots[27].stackSize, 64);
  assert.deepEqual(chest.slots[27].components, beta.components);
  assert.deepEqual(chest.slots[28].components, alpha.components);
});
